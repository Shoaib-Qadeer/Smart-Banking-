...
public class t_donation {
    ...
    t_donation()  throws SQLException {
       m.setLocationRelativeTo(null);
        String [] columnnames={"Transaction ID ","DatenTime","Type ","Account Id ","Amount","Organization"};

        Connection con = DriverManager.getConnection("jdbc:oracle:thin:@localhost:1521:xe", "bankapp", "05822");
        PreparedStatement pst = con.prepareStatement("select count(*) from transaction,donation  where transaction.id = donation .id");
        ...
        pst=con.prepareStatement("select transaction.id, transaction.datentime, transaction.type, transaction.account_account_id, donation.amount, donation.organization  from transaction,  donation where transaction.id=donation .id\n");
        ...
        m.setTitle("Donations Record Only");
